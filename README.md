...
（标「新回复」；以这台设备上是否点开过为准，手机和电脑各自记），「运行中」列出正在跑和排队的。下面是全部任务：登记了两个以上项目时按项目分段，
...
常驻进程连续 2 小时没有消息会自行退出，下一条消息再拉起。目前只有 Cursor Agent
...
如果这份列表超过 1 小时没更新、或上次拉取失败，会在后台再问一次（同一个 CLI 一分钟内最多问一次；进行中的刷新会合并成一次，模型芯片会显示「正在更新…」）。拉到的列表存在 `state.json` 里，
