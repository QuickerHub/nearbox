...
2. 打开后它常驻托盘。整个界面就是一个输入框：输入框下面的「项目」芯片选（或添加）代码目录，「Agent」芯片选谁来干、放不放开权限，选了 Agent 后旁边的「模型」芯片再挑用哪个模型；左侧是任务列表（见下）。「设置」里能看到检测到的 Agent CLI、配对手机、管理项目。
...
左侧的任务列表分两部分。顶上是两块跨项目的看板：「需要你」列出等你点允许的、运行失败的、以及跑完了但你还没点开看的任务
（标「新回复」；以这台设备上是否点开过为准，手机和电脑各自记），「进行中」列出正在跑和排队的（顶上看板只是镜像，任务自己的那一行仍留在下方对应段里）。下面是全部任务：登记了两个以上项目时按项目分段，
每段一行一条，段头标着这个目录里「1 运行 · 2 排队」，手机记进来还没派给项目的任务在「收集箱」一段；搜索框旁的按钮可以改为按时间排列。
任务开跑、跑完都只影响顶上的看板，它在下面段里的那一行不会跳来跳去。已完成的默认收起，列表底部「显示已完成」展开。手机上的列表始终按时间排列。
...
```text
手机：Agent 芯片选「只记录」，输入「给登录页加记住密码」  ──►  电脑左侧「收集箱」里出现这条
...
对话里每一轮都按 Cursor 的方式折叠：进行中时展开、实时滚动，结束后收成一行「工作了 36 秒 · 5 步」，只留 Agent 的回答。
...
其他情形自动退回到原来的「一句话一个进程」方式：远程电脑上的运行、开了「可委派」的运行、常驻进程没起来、
选了常驻会话里没有对应预设的模型（会先打开/加载会话再对照模型列表；会话仍未给出列表时才会退回）、
以及在这个版本之前开始的旧会话（命令行模式创建的会话接不进常驻进程；想要快的回复，点「改为新会话」即可）。
退回时对话里会留一行状态说明原因。
点停止时先通知这一轮取消；若 Agent 不理睬，再发 `$/cancel_request` 强制取消本轮，必要时本地结束挂起的请求；
若常驻进程上还有别的会话在跑，超时后也不会为了停这一轮而整进程杀掉。
常驻进程连续 2 小时没有消息会先按能力关闭已加载的会话，再自行退出；下一条消息再拉起。同时常驻进程里最多保留几个最近用过的会话，多出来的会在 Agent 支持时关掉，避免来回点任务把内存撑满。目前只有 Cursor Agent
...
- **安全模式**（默认）：读写项目文件直接做；终端命令停下来，让你点允许或拒绝。若一轮里同时来了多条命令询问，会排队逐条问你，不会丢掉后面的；当前这条下面会提示还剩几条。
...
如果这份列表超过 1 小时没更新、或上次拉取失败，会在后台再问一次（同一个 CLI 一分钟内最多问一次；选中 Agent 时的刷新与模型芯片共用一次请求，不会因列表状态回写再打一轮；进行中的刷新会合并成一次，模型芯片会显示「正在更新…」）。拉到的列表存在 `state.json` 里，
...
- 输入框的「停止 / 排队」只看任务自己的顶层一轮：子任务还在跑时，仍可接着和主 Agent 说话，不会被挡住。
