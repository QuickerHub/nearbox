/**
 * Split inline markdown into text / code / bold / italic / strike / links. Used
 * only to paint the conversation; the stored agent output is never rewritten.
 */
...
function piecesFor(token: string): InlinePiece[] {
  ...
  if (token.startsWith("~~") && token.endsWith("~~")) {
    return [{ type: "strike", text: token.slice(2, -2) }];
  }
  ...
  if (WWW_LINK.test(token)) {
    const { href, trail } = peelAutolink(token);
    const safe = safeHttpUrl(`https://${href}`);
    if (safe) {
