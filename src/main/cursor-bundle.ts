...
/** node.exe / index.js must exist, be regular files, and not be empty placeholders. */
...
export function resolveCursorAgentBundle(shimDir: string): CursorBundle | null {
  ...
  if (isNonEmptyRegularFile(inPlace) && isNonEmptyRegularFile(inPlaceNode)) {
  ...
  const versions = readdirSync(versionsDir, { withFileTypes: true })
    ...
    .filter(
      (name) =>
        isNonEmptyRegularFile(join(versionsDir, name, "node.exe")) &&
        isNonEmptyRegularFile(join(versionsDir, name, "index.js")),
    )
