...
/** Bundled node.exe / index.js must be real files — a directory named node.exe would pass existsSync. */
...
export function resolveCursorAgentBundle(shimDir: string): CursorBundle | null {
  ...
  if (isRegularFile(inPlace) && isRegularFile(inPlaceNode)) {
  ...
  const versions = readdirSync(versionsDir, { withFileTypes: true })
    ...
    .filter((name) => isRegularFile(join(versionsDir, name, "node.exe")) && isRegularFile(join(versionsDir, name, "index.js")))
